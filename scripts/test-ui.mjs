...
const browser = await chromium.launch({ headless: true, ...(process.env.BROWSER_EXECUTABLE_PATH ? { executablePath: process.env.BROWSER_EXECUTABLE_PATH } : {}), args: ['--enable-unsafe-swiftshader', '--autoplay-policy=no-user-gesture-required'] })
const url = process.env.UI_TEST_URL || 'http://127.0.0.1:5173'
const artifacts = new URL('../.artifacts/', import.meta.url)
await mkdir(artifacts, { recursive: true })
const context = await browser.newContext({ viewport: { width: 1440, height: 1024 }, deviceScaleFactor: 1, acceptDownloads: true })
const page = await context.newPage()
const errors = []
page.on('pageerror', error => errors.push(error.message))
const screenshot = name => page.screenshot({ path: new URL(`${name}.png`, artifacts).pathname.replace(/^\/([A-Z]:)/, '$1'), fullPage: true })
try {
  await page.goto(url)
  ...
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true, 'desktop must not overflow')

  await page.getByRole('button', { name: '收藏 Star Orbit · 星际漫游', exact: true }).click()
  await page.getByRole('button', { name: '我的收藏' }).click()
  assert.equal(await page.locator('.game-card').count(), 1, 'favorites must filter real saved games')
  ...
  rom[0xac] = 0x54; rom[0xad] = 0x53
  let checksum = 0; for (let i = 0xa0; i < 0xbd; i++) checksum = (checksum - rom[i]) & 255
  rom[0xbd] = (checksum - 0x19) & 255
  await page.locator('input[type=file][accept*=".gba"]').setInputFiles({ name: 'Test-Orbit.gba', mimeType: 'application/octet-stream', buffer: rom })
  ...
  assert.equal(await page.getByRole('combobox', { name: '画面滤镜', exact: true }).inputValue(), 'crt')
  await page.getByRole('button', { name: '控制器设置', exact: true }).click()
  await page.getByRole('button', { name: 'V', exact: true }).waitFor()
  await page.getByRole('button', { name: '关闭对话框' }).click()
  await page.getByRole('button', { name: '存档管理', exact: true }).click()
  await page.locator('.state-card').nth(1).waitFor()
  assert.equal(await page.locator('.state-card').count(), 2, 'auto and manual states persist across reload')
  ...
  await page.getByRole('button', { name: '开始试玩', exact: true }).click()
  await page.waitForFunction(() => !!document.querySelector('[aria-label="暂停 (Space)"]') && !document.querySelector('[aria-label="暂停 (Space)"]').disabled)
  ...
  console.log(JSON.stringify({ passed: true, checks: ['initial library', 'favorites', 'search', 'real ROM launch', 'keyboard', 'quick save/load', 'state download', 'key remapping', 'CRT filter', 'game import', 'reload persistence', 'mobile navigation', 'touch controls', 'responsive overflow'], screenshots: artifacts.pathname }, null, 2))
} catch (error) {
