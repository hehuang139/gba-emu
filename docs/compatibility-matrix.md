...
Advance 的 mGBA 线程核心要求页面启用跨源隔离，并依赖 SharedArrayBuffer、WebAssembly、Canvas 2D 和 IndexedDB。应用优先使用 WebGL 2；不可用时自动使用 Canvas 2D 软件渲染。发布前请在目标浏览器打开应用，点击顶部「环境检查」，记录实际图形后端和完整操作结果；面板中的存储空间只表示浏览器当前估算值。
